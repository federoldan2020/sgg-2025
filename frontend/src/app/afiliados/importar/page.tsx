...
export default function ImportarAfiliadosPage() {
  ...
  const onPreview = async () => {
    ...
    try {
      ...
      const data = await upload('/afiliados/import/preview', fd);
      setPreview(data);
  ...
  const onConfirm = async () => {
    ...
    setLoading(true); setError(null);
    try {
      const r = await fetch(`${API_URL}/afiliados/import/confirm`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Organizacion-ID': orgId, ...(token ? { Authorization: `Bearer ${token}` } : {}) },
        body: JSON.stringify({ previewId: preview.previewId, ignoreWarnings: false }),
      });
      if (!r.ok) throw new Error(await r.text());
  ...
  return (
    <div className="p-6 space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Importar Afiliados</CardTitle>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <a href={`${API_URL}/afiliados/import/template`} className="text-blue-600 underline" target="_blank" rel="noreferrer">Descargar plantilla</a>
            <a href={`${API_URL}/afiliados/import/ejemplo`} className="text-blue-600 underline" target="_blank" rel="noreferrer">Descargar ejemplo</a>
          </div>
