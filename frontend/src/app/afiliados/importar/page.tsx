...
import { api, apiBlob, apiFetch, getErrorMessage } from '@/servicios/api';
...
export default function ImportarAfiliadosPage() {
  ...
  const onConfirm = async () => {
    ...
    try {
      const data = await api<{ resumen: { creados: number; actualizados: number; errores: number }; errores?: Array<{ fila: number; mensaje: string }> }>('/afiliados/import/confirm', {
        method: 'POST',
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <Button
              variant="secondary"
              onClick={() =>
                void downloadFile('/afiliados/import/template.xlsx', 'plantilla_afiliados.xlsx')
              }
              disabled={loading}
            >
              Descargar plantilla
            ...
            <Button
              variant="secondary"
              onClick={() =>
                void downloadFile('/afiliados/import/ejemplo.xlsx', 'ejemplo_afiliados.xlsx')
              }
              disabled={loading}
            >
              Descargar ejemplo
