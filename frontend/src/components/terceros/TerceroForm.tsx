...
type RolAny = RolEditable | "AFILIADO" | "OTRO";
type CondIva =
  | ""
  | "INSCRIPTO"
  | "MONOTRIBUTO"
...
const ROLES: { value: RolEditable; label: string; icon: string }[] = [
...
export default function TerceroForm({
  ...
  mode: "crear" | "editar";
  initial?: TerceroInitial;
  onSubmit: (payload: Record<string, unknown>) => Promise<void>;
  cancelHref?: string;
}) {
  // Contacto/dirección "primarios" derivados del inicial (para edición)
  const contInit = initial?.contactos ?? [];
  const dirInit = initial?.direcciones ?? [];
  const emailInicial =
    contInit.find((c) => c.tipo === "EMAIL")?.valor ?? "";
  const telInicial =
    contInit.find((c) => c.tipo === "TELEFONO")?.valor ?? "";
  const dirPrimaria =
    dirInit.find((d) => d.principal) ?? dirInit[0] ?? undefined;

  const rolesIniciales = (initial?.roles ?? []).map((r) => r.rol);
  const rolesEditablesIniciales = rolesIniciales.filter((r) =>
    EDITABLE_ROLES.includes(r as RolEditable)
  ) as RolEditable[];
  ...
  const [iibb, setIibb] = useState(initial?.iibb ?? "");
  const [condIva, setCondIva] = useState<CondIva>(
    (initial?.condIva as CondIva) ?? ""
  );
  const [tipoPersona, setTipoPersona] = useState<TipoPersona>(
    (initial?.tipoPersona as TipoPersona) ?? ""
  );
  const [roles, setRoles] = useState<RolEditable[]>(
    mode === "crear"
      ? ["COMERCIO"]
      : rolesEditablesIniciales.length
        ? rolesEditablesIniciales
        : []
  );
  ...
  const [calle, setCalle] = useState(dirPrimaria?.calle ?? "");
  const [numero, setNumero] = useState(dirPrimaria?.numero ?? "");
  const [ciudad, setCiudad] = useState(dirPrimaria?.ciudad ?? "");
  const [provincia, setProvincia] = useState(dirPrimaria?.provincia ?? "");
  const [cp, setCp] = useState(dirPrimaria?.cp ?? "");
  ...
  const buildPayload = (): Record<string, unknown> => {
    // Roles: editables seleccionados + los preservados (AFILIADO/OTRO del inicial)
    const rolesPreservados = rolesIniciales.filter(
      (r) => !EDITABLE_ROLES.includes(r as RolEditable)
    );
    const rolesFinal = Array.from(new Set<RolAny>([...roles, ...rolesPreservados]));

    // Contactos: preservar los que no son el email/teléfono primario (ej. WEB)
    const otrosContactos = contInit.filter(
      (c) => c.valor !== emailInicial && c.valor !== telInicial
    );
    const contactos: ContactoLite[] = [...otrosContactos];
    if (email.trim())
      contactos.push({ tipo: "EMAIL", valor: email.trim(), principal: true });
    if (telefono.trim()) contactos.push({ tipo: "TELEFONO", valor: telefono.trim() });

    // Direcciones: preservar las que no son la primaria
    const otrasDirecciones = dirInit.filter((d) => d !== dirPrimaria);
    const hayDir =
      calle.trim() || ciudad.trim() || provincia.trim() || cp.trim() || numero.trim();
    const direcciones: DireccionLite[] = [...otrasDirecciones];
    if (hayDir) {
      direcciones.push({
        etiqueta: dirPrimaria?.etiqueta ?? "fiscal",
        calle: calle.trim() || undefined,
        numero: numero.trim() || undefined,
        ciudad: ciudad.trim() || undefined,
        provincia: provincia.trim() || undefined,
        cp: cp.trim() || undefined,
        pais: dirPrimaria?.pais ?? undefined,
        principal: true,
      });
    }

    // Bancos: se preservan tal cual (no se editan en este form)
    const bancos = (initial?.bancos ?? []).map((b) => ({
      banco: b.banco ?? undefined,
      tipo: b.tipo,
      numero: b.numero,
      titular: b.titular ?? undefined,
      cuitTitular: b.cuitTitular ?? undefined,
    }));
    ...
    return {
      ...
      roles: rolesFinal,
      ...(mode === "crear" && saldo != null && Number.isFinite(saldo)
  ...
  const submit = async () => {
    try {
      ...
      if (roles.length === 0) throw new Error("Seleccioná al menos un rol.");
      await onSubmit(buildPayload());
      // si onSubmit navega, no llegamos acá; si no, dejamos de postear
      setPosting(false);
    } catch (e) {
      const m =
        e instanceof Error ? e.message : "No se pudo guardar el tercero.";
      setMsg(m);
      setPosting(false);
  ...
  return (
    <div className="space-y-4">
      {msg && (
        <div
          className="flex items-center gap-3 rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800"
          role="alert"
        >
      ...
      <Card className="rounded-xl border-neutral-200 p-5">
        ...
        <div className="mt-4">
          ...
          <p className="mt-1.5 text-xs text-neutral-400">
            ...
            {mode === "editar" &&
              rolesIniciales.some((r) => !EDITABLE_ROLES.includes(r as RolEditable)) &&
              " Los roles internos (afiliado/otro) se conservan."}
          </p>
        </div>
      </Card>

      {/* Saldo inicial (solo alta) */}
      {mode === "crear" && (
        <Card className="rounded-xl border-neutral-200 p-5">
          <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold uppercase tracking-wider text-neutral-500">
