...
import { ImportOptionsDto, ConfirmImportDto } from './dto/import-parentescos.dto';

@Controller('parametricos/parentescos/import')
@UseGuards(JwtAuthGuard)
export class ParentescosImportController {
  constructor(private readonly importService: ParentescosImportService) {}
  ...
  descargarPlantilla(@Res() res: Response) {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="plantilla_parentescos.csv"');
    res.send(csv);
  }
  ...
  async descargarPlantillaXlsx(@Res() res: Response) {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="plantilla_parentescos.xlsx"');
    res.send(xlsx);
  }
  ...
  descargarEjemplo(@Res() res: Response) {
    ...
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', 'attachment; filename="ejemplo_parentescos.csv"');
    res.send(csv);
  }
  ...
  async descargarEjemploXlsx(@Res() res: Response) {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="ejemplo_parentescos.xlsx"');
    res.send(xlsx);
  }
